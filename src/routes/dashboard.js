...
async function load() {
  try {
    ...
    document.getElementById('hitBar').style.width = d.requests.hitRate + '%';
    set('uptime', formatUptime(d.uptime));

    const dot = d.redis.status === 'ok'
      ? '<span class="status-dot dot-green"></span>OK'
      : '<span class="status-dot dot-red"></span>Error';
    document.getElementById('redisStatus').innerHTML = dot;
    document.getElementById('rStatus').innerHTML    = dot;
    ...
    if (!d.recent.length) {
      tbody.innerHTML = '<tr><td colspan="5" style="color:#64748b;text-align:center;padding:1rem">მოთხოვნები არ არის</td></tr>';
    } else {
      tbody.innerHTML = d.recent.map(r => {
        const dClass = r.durationMs < 200 ? 'text-green' : r.durationMs > 5000 ? 'text-red' : '';
        const cClass = r.cacheHit ? 'badge-green' : 'badge-red';
        const cLabel = r.cacheHit ? 'HIT' : 'MISS';
        return '<tr>' +
...
<style>
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .hit-bar { height: 100%; border-radius: 999px; background: #22c55e; transition: width 0.4s ease; }
...
<main>
  <div class="grid-4">
    ...
    <div class="card">
      ...
      <div class="hit-bar-wrap"><div class="hit-bar" id="hitBar" style="width:0%"></div></div>
    ...
    <div class="card">
      ...
      <div class="card-value" style="font-size:1.1rem;padding-top:0.4rem" id="redisStatus">—</div>
    </div>
  </div>

  <div class="grid-2">
    <div class="card">
      <div class="card-label" style="margin-bottom:0.75rem">Redis ინფო</div>
    ...
    <div class="card">
      <div class="card-label" style="margin-bottom:0.75rem">Rate Limit</div>
  ...
  <div class="card">
    <div class="card-label" style="margin-bottom:0.75rem">ბოლო მოთხოვნები</div>
    <table>
      <thead>
        <tr><th>ტექსტი</th><th>ენა</th><th>Cache</th><th>დრო</th><th>თარიღი</th></tr>
      </thead>
      <tbody id="recentTable">
        <tr><td colspan="5" style="color:#64748b;text-align:center;padding:1rem">ჩატვირთვა...</td></tr>
      </tbody>
    </table>
  </div>
</main>
